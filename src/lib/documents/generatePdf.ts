...
function formatMoneyForPdf(n: number | null | undefined): string {
  if (n === null || n === undefined) return "Not specified";
  return `$${n.toLocaleString("en-US")}`;
}

function formatDateForPdf(d: string | Date | null | undefined): string {
...
/**
 * One-page-plus reference sheet for a single tracked grant: everything a
 * client would otherwise have to screenshot from the grant detail page,
 * as a clean downloadable PDF (see "Download PDF" on the grant detail
 * page and grants list). Reuses the same page/wrap machinery as
 * generateDocumentPdf above.
 */
export async function generateGrantSummaryPdf(grant: GrantSummaryInput): Promise<Uint8Array> {
  const pdf = await PDFDocument.create();
  const font = await pdf.embedFont(StandardFonts.Helvetica);
  const boldFont = await pdf.embedFont(StandardFonts.HelveticaBold);

  pdf.setTitle(grant.title || "Grant Summary");
  ...
  function drawLabelValue(label: string, value: string) {
    ...
    page.drawText(label, { x: MARGIN, y, size: 9.5, font: boldFont, color: rgb(0.4, 0.4, 0.45) });
    y -= LINE_HEIGHT * 0.85;
    for (const line of wrapLine(value, font, BODY_SIZE, CONTENT_WIDTH)) {
      ensureSpace(LINE_HEIGHT);
      page.drawText(line, { x: MARGIN, y, size: BODY_SIZE, font, color: rgb(0.1, 0.1, 0.1) });
      y -= LINE_HEIGHT;
    }
  ...
  function drawParagraphSection(heading: string, text: string) {
    ensureSpace(LINE_HEIGHT * 3);
    drawHeading(heading, 13);
    for (const paragraph of text.split(/\n\s*\n/)) {
      for (const rawLine of paragraph.split("\n")) {
        for (const line of wrapLine(rawLine, font, BODY_SIZE, CONTENT_WIDTH)) {
          ensureSpace(LINE_HEIGHT);
  ...
  drawHeading(grant.title || "Untitled Grant", TITLE_SIZE);
  if (grant.agency) {
    y -= 2;
    ensureSpace(LINE_HEIGHT);
    page.drawText(grant.agency, { x: MARGIN, y, size: 12, font, color: rgb(0.35, 0.35, 0.4) });
    y -= LINE_HEIGHT * 1.3;
  }
  y -= 6;
  ...
  const awardRange =
    grant.awardFloor || grant.awardCeiling
      ? `${formatMoneyForPdf(grant.awardFloor)} - ${formatMoneyForPdf(grant.awardCeiling)}`
      : "Not specified";

  drawLabelValue("STATUS", grant.status || "Unknown");
  if (grant.category) drawLabelValue("CATEGORY", grant.category);
  drawLabelValue("AWARD RANGE", awardRange);
  drawLabelValue("OPENS", formatDateForPdf(grant.openDate));
  drawLabelValue("DEADLINE", formatDateForPdf(grant.deadline));
  if (grant.eligibleApplicants) drawLabelValue("ELIGIBLE APPLICANTS", grant.eligibleApplicants);
  if (grant.url) drawLabelValue("ORIGINAL LISTING", grant.url);

  y -= 4;

  if (grant.summary) drawParagraphSection("Summary", grant.summary);
  if (grant.aiSummary) drawParagraphSection("AI Analysis", grant.aiSummary);
  ...
  pages.forEach((p, i) => {
    ...
    p.drawText("Generated by Grant Scout Pro", {
      x: MARGIN,
      y: MARGIN / 2,
      size: 8,
      font,
