...
export default function RenewalWorkspacePage({
  ...
  return (
    <div className="space-y-8">
      ...
      <RenewalEligibility
        eligible={
          renewalData?.eligibility?.eligible
        }
        reason={
          renewalData?.eligibility?.reasons?.join(
            ", "
          ) ?? ""
        }
      />
      ...
      <RenewalProbability
        score={renewalData.probability}
      />

      <RenewalNarrative />

      <RenewalBudgetUpdate
        budget={renewalData.budget}
      />

      <AIRenewalRecommendations
        insights={
          renewalData.aiRecommendations
        }
      />
    </div>
  );
