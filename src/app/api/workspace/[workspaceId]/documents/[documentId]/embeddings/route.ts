...
export async function GET(req: NextRequest, context: { params: Promise<Record<string,string>> }) {
  try {
    ...
    return NextResponse.json({ success: true, method: "GET", documentId, workspaceId });
  } catch (err: any) {
...
export async function POST(req: NextRequest, context: { params: Promise<Record<string,string>> }) {
  try {
    ...
    return NextResponse.json({ success: true, method: "POST", documentId, workspaceId, body });
  } catch (err: any) {
    console.error("POST EMBEDDINGS ERROR:", err);
    return NextResponse.json({ error: err?.message }, { status: 500 });
  }
}
