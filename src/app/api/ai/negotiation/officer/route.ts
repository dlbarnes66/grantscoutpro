...
// POST /api/ai/negotiation/officer  { workspaceId, grantId, orgNotes? }
export async function POST(req: NextRequest) {
  ...
  const userPrompt = `
Prepare talking points for a call/meeting with this grant's program officer.
...
- officerTalkingPoints (array of strings)
- leverageArguments (array of strings)
- riskWarnings (array of strings)
- recommendedTone (string)
`.trim();

  const { parsed, raw } = await generateNegotiationSection(SYSTEM_PROMPT, userPrompt);
  await saveNegotiationSection(userId, grantId, "officer", raw);
